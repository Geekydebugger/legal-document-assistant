...
def load_agent():
    ...
    DOCUMENTS = [
        ...
        {"id": "doc_012", "topic": "Non-Compete and Restraint of Trade Clauses", "text": "Non-compete clauses restrict former employees from working for competitors for a defined period and geographic area, typically 6-24 months. Enforceability requires reasonableness in duration, geographic area, and scope. Overly broad clauses are struck down by courts. Non-solicitation clauses are narrower: they prevent approaching former clients or recruiting employees. California largely prohibits non-compete clauses; they are more enforceable in UK and India."},
    ]
        ...
        try:
            ...
            if date_match and days_match:
                ...
                tool_result = (f"Date Calculation:\nStart: {start_date.strftime('%B %d, %Y')}\n"
                               f"Duration: {days} days\nEnd date: {end_date.strftime('%B %d, %Y')} ({end_date.strftime('%A')})")
            else:
